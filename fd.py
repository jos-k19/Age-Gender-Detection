...
prototxtPath = os.path.sep.join(["face_detector", "opencv_face_detector.pbtxt"])
weightsPath = os.path.sep.join(["face_detector","opencv_face_detector_uint8.pb"])
...
print("[INFO] loading age detector model...")
prototxtPath = os.path.sep.join(["gender_detector", "gender_deploy.prototxt"])
...
for i in range(0, detections.shape[2]):
	...
	if confidence > args["confidence"]:
		...
		text1 = "{}: {:.2f}%".format(age, ageConfidence * 100)
		print("[INFO] {}".format(text1))
		...
		# display the predicted age to our terminal
		text2 = "{}: {:.2f}%".format(gender, genderConfidence * 100)
		print("[INFO] {} ".format(text2))

		text3 = "{}: {}".format(gender, age)
		...
		cv2.rectangle(image, (startX, startY), (endX, endY),
			(0, 0, 255), 2)
		cv2.putText(image, text3, (startX, y),
